...
def re_reference_npz(input_npz_folder: str, output_npz_folder: str, ignore_channels: [list, str]=None, rr_channel: int=None) -> None:
    ...
    print(f'Average re-referencing NPZ files in {input_npz_folder} and saving to {output_npz_folder}...')
    ...
    data_io.write_separate_npz_files(data_all_rereferenced, fs, output_npz_folder)


def re_reference(data: np.ndarray, ignore_channels: [list, str]=None, rr_channel: int=None) -> np.ndarray:
    ...
    if ignore_channels is not None:
        ...
        channels_list = [i for i in range(data.shape[0]) if i not in ignore_channels] 
    else:
    ...
    if rr_channel is not None:
        data_rereferenced[channels_list, :] = data[channels_list, :] - data[rr_channel, :].reshape(1, -1)
    else:
        data_rereferenced[channels_list, :] = data[channels_list, :] - np.mean(data[channels_list, :], axis=0).reshape(1, -1)
