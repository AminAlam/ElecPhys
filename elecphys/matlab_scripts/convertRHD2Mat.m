function convertRHD2Mat(files_path, save_file_name, ds_ratio)

    cd(files_path);
    files_all = dir;
    mat_all = [];
    for file_index = 1:length(files_all)
    
        file = files_all(file_index).name;
        if ~contains(file, 'rhd')
            continue
        end
        
        path = files_all(file_index).folder;
        path = path;
        ...
        fprintf(1, '\n');
        amplifier_data_ds = downsample(amplifier_data', ds_ratio);
        amplifier_data_ds = amplifier_data_ds';
        mat_all = [mat_all, amplifier_data_ds];
    end
     if rem(sample_rate, ds_ratio) ~= 0
         error('sample_rate/ds_ratio must be an integer')
     end
